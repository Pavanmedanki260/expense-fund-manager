...
export default function GroupDashboard() {
  ...
  useEffect(() => {
    if (groupId) {
      api.get(`/api/groups/${groupId}/dashboard`).then(setData).catch(console.error).finally(() => setLoading(false));
    }
  }, [groupId]);
  ...
  return (
    <div data-testid="dashboard-page">
            ...
            <ResponsiveContainer width="100%" height={260}>
              <PieChart>
                <Pie data={data.category_breakdown} dataKey="amount" nameKey="category" cx="50%" cy="50%" outerRadius={90} label={({ category, percent }) => `${category} ${(percent * 100).toFixed(0)}%`}>
                  {data.category_breakdown.map((_, i) => <Cell key={i} fill={CATEGORY_COLORS[i % CATEGORY_COLORS.length]} />)}
                </Pie>
      ...
      <div className="card" data-testid="recent-activity" style={{ marginTop: 16 }}>
